...
class PackageController extends Controller
{
    ...
    { {
            $packages = Package::join('services', 'packages.service_id', '=', 'services.id')->select('packages.*', 'services.name as sname', 'services.position as sposition')->where('packages.id', '!=', 0)->orderBy('services.position')->orderBy('packages.position')->getQuery()->get();
        }
    ...
    {
        $apis = API::all();
        $services = Service::where(['status' => 'ACTIVE'])->orderBy('position')->get();
    ...
    public function ajaxPost(\Illuminate\Http\Request $request)
    {
        ...
        if (($preferred_api_id) != NULL) {
            ...
            if (!empty($insert)) {
                ApiMapping::where(array("package_id" => $id))->delete();
                \DB::table("api_mappings")->insert($insert);
            }
        }
        \Illuminate\Support\Facades\Session::flash('alert', __('messages.created'));
    ...
    public function edit($id)
    {
        ...
        $api_package_id = ApiMapping::where(['package_id' => $id])->pluck('api_package_id')->toArray();
