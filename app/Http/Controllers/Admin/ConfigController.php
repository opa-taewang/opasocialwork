...
class ConfigController extends Controller
{
    ...
    public function update(\Illuminate\Http\Request $request)
    {
        ...
        config(['services.google.client_id' => $request->input('google_client_id')]);
        config(['services.google.client_secret' => $request->input('google_client_secret')]);
        $fp = fopen(base_path() . '/config/services.php', 'w');
        fwrite($fp, '<?php return ' . var_export(config('services'), true) . ';');
        fclose($fp);
